...
class No2BounceVerifier(Verifier):
    """No2Bounce real-time API (Reacher-compatible check_email)."""
    ...
    def __init__(
        ...
        api_key: str,
        *,
        base_url: str = "https://api.reacher.email/v0/check_email",
        ...
        self.billed_statuses = set(billed_statuses or ["valid", "invalid", "catchall"])
        self._client = client
    ...
    async def verify(self, email: str) -> Verdict:
        ...
        reachable = str(payload.get("is_reachable") or payload.get("result") or "unknown").lower()
        smtp = payload.get("smtp") or {}
        if isinstance(smtp, dict) and smtp.get("is_catch_all"):
            status = "catchall"
        else:
            status = _REACHABLE_MAP.get(reachable, "unknown")
